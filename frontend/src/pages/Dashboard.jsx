...
const Dashboard = () => {
  ...
  const stats = useMemo(() => {
    // Use backend stats if available, otherwise return empty
    if (backendStats) {
      ...
      return [
        {
          ...
          color: '#ec4899',
          subtitle: `${backendStats.totalEvents || 0} events`
        },
        {
          title: 'Total Quotations',
          value: String(backendStats.totalQuotations || 0),
          icon: <FaFileInvoiceDollar />,
          color: '#8b5cf6',
          subtitle: `${backendStats.totalQuotations || 0} quotations`
        },
        {
          title: 'Total Bills',
          value: String(backendStats.totalBills || 0),
          icon: <FaReceipt />,
          color: '#6366f1',
          subtitle: `${backendStats.totalBills || 0} invoices`
        },
        {
          title: 'Revenue (Paid)',
          value: inrFormatter.format(backendStats.revenue || 0),
          icon: <FaRupeeSign />,
          color: '#10b981',
    ...
    const totalBills = Array.isArray(invoices) ? invoices.length : 0;
    const totalEvents = Array.isArray(events) ? events.length : 0;
    const revenue = Array.isArray(invoices)
      ? invoices.reduce((sum, inv) => sum + (parseFloat(inv.paid) || 0), 0)
      : 0;
    ...
    return [
      ...
      {
        title: 'Total Quotations',
        value: String(Array.isArray(quotations) ? quotations.length : 0),
        icon: <FaFileInvoiceDollar />,
        color: '#8b5cf6',
        subtitle: `${Array.isArray(quotations) ? quotations.length : 0} quotations`
      },
      {
        title: 'Total Bills',
        ...
        icon: <FaReceipt />,
        color: '#6366f1',
        subtitle: `${totalBills} invoices`
      },
      {
        title: 'Revenue (Paid)',
        value: inrFormatter.format(revenue),
        icon: <FaRupeeSign />,
        color: '#10b981',
        subtitle: `${invoices.length} invoices`
      }
    ];
  }, [backendStats, invoices, events, quotations]);
...
const ActionButtons = () => {
  ...
  const openPolicy = () => navigate('/policy', { state: { openAdd: true } });
  ...
  return (
    <>
      ...
      <button className="action-btn" onClick={openPolicy} aria-label="Policy">
        <FaProjectDiagram />
        <span>Policy</span>
      </button>
